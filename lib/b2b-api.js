...
const DEFAULT_BASE_URL =
  process.env.NEXT_PUBLIC_B2B_BASE_URL ??
...
// -----------------------------------------------------------------------------
// Helpers – persistence & sync
// -----------------------------------------------------------------------------
function persistSession() {
  if (typeof window === "undefined") return; // SSR safeguard
  localStorage.setItem(
    "b2bAuth",
    JSON.stringify({ accessToken, refreshToken, tokenExpiry })
  );
  scheduleRefresh();
}
...
function restoreSession() {
  ...
  try {
    const saved = localStorage.getItem("b2bAuth");
    if (!saved) return;

    const { accessToken: at, refreshToken: rt, tokenExpiry: te } = JSON.parse(
      saved
    );
    accessToken = at;
    refreshToken = rt;
    ...
    localStorage.removeItem("b2bAuth"); // corrupted JSON → nuke it
  }
}
...
function clearSession() {
  ...
  tokenExpiry = 0;
  clearTimeout(refreshTimer);
  if (typeof window !== "undefined") localStorage.removeItem("b2bAuth");
}

// Live cross-tab sync
if (typeof window !== "undefined") {
  window.addEventListener("storage", (e) => {
    if (e.key !== "b2bAuth" || !e.newValue) return;
    try {
      const { accessToken: at, refreshToken: rt, tokenExpiry: te } = JSON.parse(
        e.newValue
      );
...
export const b2bApi = {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Verb helpers (JSON body by default)
  // ──────────────────────────────────────────────────────────────────────────
...
// -----------------------------------------------------------------------------
// Initialise from any stored session immediately
// -----------------------------------------------------------------------------
restoreSession();
